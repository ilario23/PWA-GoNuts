...
test.describe('Transaction Flow', () => {
  ...
  test.describe('Desktop', () => {
    ...
    test('should add transaction via Top Button', async ({page}) => {
      ...
      await expect(dialog).not.toBeVisible();
      const row = page
        .getByTestId('desktop-dashboard-grid')
        .getByRole('row')
        .filter({hasText: 'Desktop Transaction'});
      await expect(row).toBeVisible();
    ...
    test('should edit transaction via Hover & Click', async ({page}) => {
      ...
      const row = page
        .getByTestId('desktop-dashboard-grid')
        .getByRole('row')
        .filter({hasText: 'Desktop Edit Tx'})
        .last();
      ...
      // 5. Verify
      await expect(
        page.getByTestId('save-transaction-button'),
      ).not.toBeVisible();
