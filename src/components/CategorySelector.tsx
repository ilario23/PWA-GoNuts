...
export function CategorySelector({ value, onChange, type, excludeId, modal = false }: CategorySelectorProps) {
    ...
    const buildTree = React.useCallback((cats: Category[], parentId: string | undefined = undefined, level = 0): CategoryNode[] => {
        return cats
            .filter(c => {
    ...
    const getDescendantIds = React.useCallback((categoryId: string, cats: Category[]): string[] => {
    ...
    const renderCategoryIcon = (iconName: string, color: string) => {
        ...
        return (
            <div className="flex h-6 w-6 items-center justify-center rounded-full mr-2" style={{ backgroundColor: color }}>
        ...
        <div className="w-full">
            ...
            <div className="p-2 space-y-1 max-h-[300px] overflow-y-auto">
                ...
                {excludeId && navigationPath.length === 0 && (
                    ...
                    >
                        <span className="italic text-muted-foreground">{t('no_parent') || "No Parent"}</span>
        ...
        <div className="flex flex-col h-full">
            {navigationPath.length > 0 && (
                <div className="flex items-center p-4 border-b">
                    ...
                    <span className="ml-2 font-semibold">{currentParent?.name}</span>
                </div>
            )}
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
                ...
                {excludeId && navigationPath.length === 0 && (
                    <div
                        ...
                        onClick={() => handleSelect(null)}
                    >
                        <span className="flex-1 font-medium italic text-muted-foreground">{t('no_parent') || "No Parent"}</span>
