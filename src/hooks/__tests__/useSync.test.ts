...
describe('useSync', () => {
  ...
  it('should subscribe to sync changes on mount', () => {
    ...
    expect(typeof (syncManager.onSyncChange as jest.Mock).mock.calls[0][0]).toBe('function');
  });

  it('should unsubscribe from sync changes on unmount', () => {
    const unsubscribeMock = jest.fn();
    (syncManager.onSyncChange as jest.Mock).mockReturnValueOnce(unsubscribeMock);
  ...
  it('should update status when syncManager emits changes', async () => {
    ...
    const statusCallback = (syncManager.onSyncChange as jest.Mock).mock.calls[0][0];
    ...
    act(() => {
      statusCallback({
        ...
        errorCount: 2,
        errors: [
          { key: 'err1', table: 'transactions', operation: 'push', message: 'Network error', attempts: 1, lastAttempt: '2024-01-15T10:00:00Z' },
        ],
      });
    });
